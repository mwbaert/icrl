...
class LNN(nn.Module):
    def __init__(self, num_inputs):
        ...
        self.and2 = DynamicAnd(num_inputs=2, alpha=0.75)
        self.and3 = DynamicAnd(num_inputs=2, alpha=0.75)
    ...
    def forward(self, x):
        ...
        y2 = self.and2(
            torch.cat((x[:, 3][:, None], x[:, 4][:, None]),dim=-1))[:, None]
        y3 = self.and3(
            torch.cat((x[:, 1][:, None], x[:, 7][:, None]), dim=-1))[:, None]
        # y4 = self.and4(
        ...
        return self.or1(torch.cat((y3, y2), dim=-1))[:, None]
