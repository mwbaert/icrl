...
def run_policy(args):
    ...
    def make_env():
        ...
        if not config.dont_normalize_obs:
            env = VecNormalize.load(os.path.join(
                load_dir, "train_env_stats.pkl"), env)
    ...
    if not args.dont_save_trajs:
        ...
        while True:
            ...
            if not args.save_using_airl_scheme:
                ...
                saving_dict = dict(
                    observations=observations, actions=actions, rewards=rewards, lengths=lengths)
            ...
            if (args.reward_threshold is None or np.mean(saving_dict['rewards']) >= args.reward_threshold) and\
               (args.length_threshold is None or np.mean(saving_dict['lengths']) >= args.length_threshold):
                print(
                    f"{idx}. Mean reward: {np.mean(saving_dict['rewards'])} | Mean length: {np.mean(saving_dict['lengths'])}")
...
def main():
    ...
    parser.add_argument("--save_using_airl_scheme",
