...
from icrl.l_constraint_net import LogicConstraintNet
    ...
    for i in range(num_rollouts):
        ...
        if i == 0:
            ...
            expert_obs = np.concatenate(
                [expert_obs, data['observations']], axis=0)
...
def icrl(config):
    ...
    # )
    constraint_net = LogicConstraintNet(
        obs_dim,
        acs_dim,
        action_low=action_low,
        action_high=action_high)
    ...
    #    obs_dim, acs_dim, os.path.join(
    #        config.save_dir, "true_constraint_net.png"),
    #    observations=obs
    # )

    # Initialize agent
    def create_nominal_agent(): return PPOLagrangian(
    ...
    print(utils.colorize("\nBeginning training",
                         color="green", bold=True), flush=True)
    best_true_reward, best_true_cost, best_forward_kl, best_reverse_kl = - \
        np.inf, np.inf, np.inf, np.inf
    for itr in range(config.n_iters):
        if config.reset_policy and itr != 0:
            print(utils.colorize("Resetting agent",
                                 color="green", bold=True), flush=True)
            nominal_agent = create_nominal_agent()
        ...
        if itr % config.cn_plot_every == 0:
            ...
            if config.clip_obs is not None:
                obs_for_plot = np.clip(
                    obs_for_plot, -config.clip_obs, config.clip_obs)
            ...
            if config.train_env_id == 'AntWall-v0':
                plot_obs_ant(obs_for_plot, os.path.join(
                    other_plot_dir, "%d.png" % itr))
        ...
        average_true_cost = np.mean(
            true_cost_function(orig_observations, actions))
        ...
        if itr % config.save_every == 0:
            ...
            if isinstance(train_env, VecNormalize):
                train_env.save(os.path.join(
                    path, f"{itr}_train_env_stats.pkl"))
        # (2): Best model
        if average_true_reward > best_true_reward:
            print(utils.colorize("Saving new best model",
                                 color="green", bold=True), flush=True)
            nominal_agent.save(os.path.join(
                config.save_dir, "best_nominal_model"))
            constraint_net.save(os.path.join(
                config.save_dir, "best_cn_model.pt"))
            if isinstance(train_env, VecNormalize):
                train_env.save(os.path.join(
                    config.save_dir, "train_env_stats.pkl"))
        ...
        if config.verbose > 0:
            icrl_logger.write(
                metrics, {k: None for k in metrics.keys()}, step=itr)
        wandb.log(metrics)
...
def main():
    ...
    parser.add_argument("--policy_layers", "-pl", type=int,
                        default=[64, 64], nargs='*')
    parser.add_argument("--reward_vf_layers", "-rvl",
                        type=int, default=[64, 64], nargs='*')
    parser.add_argument("--cost_vf_layers", "-cvl",
                        type=int, default=[64, 64], nargs='*')
    # ========================= Training ============================ #
    ...
    parser.add_argument("--warmup_timesteps", "-wt",
                        type=lambda x: int(float(x)), default=None)
    parser.add_argument("--forward_timesteps", "-ft",
                        type=lambda x: int(float(x)), default=1e6)
    parser.add_argument("--backward_iters", "-bi", type=int, default=10)
    parser.add_argument('--no_importance_sampling',
                        '-nis', action='store_true')
    parser.add_argument('--per_step_importance_sampling',
    ...
    parser.add_argument("--cn_target_kl_old_new",
                        "-ctkon", type=float, default=10)
    parser.add_argument("--cn_target_kl_new_old",
                        "-ctkno", type=float, default=10)
    parser.add_argument("--cn_eps", "-ce", type=float, default=1e-5)
    # ======================== Expert Data ========================== #
    parser.add_argument('--expert_path', '-ep', type=str,
                        default='icrl/expert_data/HCWithPos-vm0')
