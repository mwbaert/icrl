...
class RedGreenEnv(mujoco_env.MujocoEnv):
    metadata = {"render_modes": ["rgb_array"]}

    def __init__(self, constraint_regions=[], constraint_state_action_pairs=[], track_agent=False, normalize_obs=False, constrained=False):
        # Environment setup.
        self.constrained = constrained
        self.size = GRID_SIZE
        self.max_time_steps = TEST_MAX_TIME_STEPS
        self.start_pos = np.array([[0, 5]])
        self.goal_pos = np.array([[11, 5]])
        ...
        self.obs_min = np.array([0, 0, 0, 0])
        self.obs_max = np.array(
            [GRID_SIZE-1, GRID_SIZE-1, 4, 1])
    ...
    def reward(self, state, action):
        ...
        if self.constrained and (state[-1] == 1) and (action != 0):
            reward = -100
        elif(next_state[:STATE_Y+1] == self.goal).all():
    ...
    def template(self):
        ...
        # Add constraints.
        if self.constraint_regions != None:
    ...
    def render(self, *args, **kwargs):
        ...
        if hasattr(self, 'curr_state'):
            ...
            # add traffic light
            if self.curr_state[STATE_LIGHT]:
               add_circle(ax, [1, 1], 'r', 0.4, False)
            else:
               add_circle(ax, [1, 1], 'g', 0.4, False)
        else:
    ...
    def applyAction(self, state, action):
        ...
        if random.random() < 0.5:
           next_state[STATE_LIGHT] = 1
        else:
           next_state[STATE_LIGHT] = 0
...
class ConstrainedRedGreenEnv(RedGreenEnv):
    def __init__(self, *args):
        ...
        super().__init__(constraint_regions, None, constrained=True, *args)
